...
function skillLight(player) {
    ...
    world.getNearbyEntities(player.getLocation(), RANGE, RANGE, RANGE).forEach(function (ent) {
        if (ent instanceof LivingEntity) {
            ...
            if (nightVisionType) {
                ent.addPotionEffect(new PotionEffect(nightVisionType, BUFF_TICKS, 0));
            }
            if (waterBreathingType) {
                ent.addPotionEffect(new PotionEffect(waterBreathingType, BUFF_TICKS, 0));
            }
            if (glowingType) {
                ent.addPotionEffect(new PotionEffect(glowingType, 200, 0)); // 10秒发光
            }
            
            // 光明粒子效果（安全生成）
            if (glowParticle) {
                world.spawnParticle(glowParticle, ent.getLocation(), 10, 1, 2, 1, 0.1);
            }
        }
    });
...
function skillCourage(player) {
    ...
    world.getNearbyEntities(player.getLocation(), RANGE, RANGE, RANGE).forEach(function (ent) {
        if (ent instanceof LivingEntity) {
            ...
            if (strengthType) {
                ent.addPotionEffect(new PotionEffect(strengthType, BUFF_TICKS, STRENGTH_LEVEL));
            }
            if (resistanceType) {
                ent.addPotionEffect(new PotionEffect(resistanceType, BUFF_TICKS, RESISTANCE_LEVEL));
            }
            if (healthBoostType) {
                ent.addPotionEffect(new PotionEffect(healthBoostType, BUFF_TICKS, 1));
            }
            
            // 勇气粒子效果（安全生成）
            if (critParticle) {
                world.spawnParticle(critParticle, ent.getLocation(), 8, 1, 2, 1, 0.1);
            }
        }
    });
...
function skillWisdom(player) {
    ...
    world.getNearbyEntities(player.getLocation(), RANGE, RANGE, RANGE).forEach(function (ent) {
        if (ent instanceof LivingEntity) {
            ...
            if (luckType) {
                ent.addPotionEffect(new PotionEffect(luckType, BUFF_TICKS, 1));
            }
            if (hasteType) {
                ent.addPotionEffect(new PotionEffect(hasteType, BUFF_TICKS, 1));
            }
            if (saturationType) {
                ent.addPotionEffect(new PotionEffect(saturationType, 100, 0)); // 5秒饱和
            }
            
            // 智慧粒子效果（安全生成）
            if (enchantParticle) {
                world.spawnParticle(enchantParticle, ent.getLocation(), 15, 1, 2, 1, 0.2);
            }
        }
    });
