const DISPLAY_NAME = '§a消§b耗§9卡 §d- §6不祥之兆 IV';
...
function onUse(evt) {
    ...
    player.sendMessage(`§6[${SKILL_NAMES[skillIndex]}] §a已使用！`);
...
function skillBuff(player) {
    ...
    //提供不祥之兆效果
    const badOmenEffect = PotionEffectType.getByName("BAD_OMEN") || PotionEffectType.BAD_OMEN;

    const buffs = [
        new PotionEffect(badOmenEffect, BUFF_TICKS, 3)
    ];
