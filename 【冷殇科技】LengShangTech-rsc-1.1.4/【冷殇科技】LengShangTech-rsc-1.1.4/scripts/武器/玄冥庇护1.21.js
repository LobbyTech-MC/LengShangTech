...
function skillHealRain(player) {
    ...
    const Player = Packages.org.bukkit.entity.Player;   // 只选玩家
...
function skillShieldZone(player) {
    ...
    world.getNearbyEntities(player.getLocation(), RANGE, RANGE, RANGE).forEach(function (ent) {
        if (ent instanceof Player) {
            ...
            if (absorptionType) ent.addPotionEffect(new PotionEffect(absorptionType, SHIELD_TICKS, 2));
            if (resistanceType) ent.addPotionEffect(new PotionEffect(resistanceType, SHIELD_TICKS, 1));
            if (fireResistanceType) ent.addPotionEffect(new PotionEffect(fireResistanceType, SHIELD_TICKS, 0));
...
function skillSwiftWind(player) {
    ...
    world.getNearbyEntities(player.getLocation(), RANGE, RANGE, RANGE).forEach(function (ent) {
        if (ent instanceof Player) {
            ...
            if (speedType) ent.addPotionEffect(new PotionEffect(speedType, SPEED_TICKS, SPEED_LEVEL));
            if (jumpType) ent.addPotionEffect(new PotionEffect(jumpType, JUMP_TICKS, JUMP_LEVEL));
