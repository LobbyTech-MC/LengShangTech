...
function skillBuff(player) {
    ...
    const buffs = [
        new PotionEffect(PotionEffectType.INCREASE_DAMAGE, BUFF_TICKS, 4),
        new PotionEffect(PotionEffectType.SPEED, BUFF_TICKS, 4),
        new PotionEffect(PotionEffectType.REGENERATION, BUFF_TICKS, 4)
    ];

    world.getNearbyEntities(player.getLocation(), RANGE, RANGE, RANGE).forEach(function (ent) {
        if (ent instanceof LivingEntity && ent !== player) {
            ent.addPotionEffect(buffs[0]);
            ent.addPotionEffect(buffs[1]);
            ent.addPotionEffect(buffs[2]);
        }
    });
    // 自己也吃
    buffs.forEach(b => player.addPotionEffect(b));
...
function skillShield(player) {
    ...
    const negatives = [
        ...
        PotionEffectType.BLINDNESS, PotionEffectType.SLOW
    ];

    world.getNearbyEntities(player.getLocation(), RANGE, RANGE, RANGE).forEach(function (ent) {
        if (ent instanceof LivingEntity) {
            negatives.forEach(t => ent.removePotionEffect(t));
            // 抗性 + 吸收护盾
            ent.addPotionEffect(new PotionEffect(PotionEffectType.DAMAGE_RESISTANCE, SHIELD_TICKS, 2));
            ent.addPotionEffect(new PotionEffect(PotionEffectType.ABSORPTION, SHIELD_TICKS, 10));
        }
    });
