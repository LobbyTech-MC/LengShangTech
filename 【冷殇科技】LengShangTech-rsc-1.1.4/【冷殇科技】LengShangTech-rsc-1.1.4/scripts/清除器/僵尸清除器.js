...
function onUse(evt) {
    ...
    /* ===== 清除“普通”僵尸 ===== */
    const Zombie = Packages.org.bukkit.entity.Zombie;
    let clearCount = 0;

    world.getNearbyEntities(location, RANGE, RANGE, RANGE).forEach(function(ent) {
        if (!(ent instanceof Zombie)) return;          // 只处理僵尸
    ...
    // 反馈信息
    if (clearCount > 0) {
        player.sendMessage(`§6已清除${clearCount}只僵尸`);
    } else {
        player.sendMessage('§7半径10格内未发现可清除的僵尸');
    }
}
