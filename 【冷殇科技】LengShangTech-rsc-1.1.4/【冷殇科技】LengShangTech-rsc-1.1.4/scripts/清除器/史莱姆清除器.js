...
function onUse(evt) {
    ...
    /* ===== 清除“普通”史莱姆 ===== */
    const Slime = Packages.org.bukkit.entity.Slime;
    let clearCount = 0;

    world.getNearbyEntities(location, RANGE, RANGE, RANGE).forEach(function(ent) {
        if (!(ent instanceof Slime)) return;          // 只处理史莱姆
    ...
    // 反馈信息
    if (clearCount > 0) {
        player.sendMessage(`§6已清除${clearCount}只史莱姆`);
    } else {
        player.sendMessage('§7半径10格内未发现可清除的史莱姆');
    }
}
