...
function onUse(evt) {
    ...
    /* ===== 清除“普通”幻翼 ===== */
    const Phantom = Packages.org.bukkit.entity.Phantom;
    let clearCount = 0;

    world.getNearbyEntities(location, RANGE, RANGE, RANGE).forEach(function(ent) {
        if (!(ent instanceof Phantom)) return;          // 只处理幻翼
    ...
    // 反馈信息
    if (clearCount > 0) {
        player.sendMessage(`§6已清除${clearCount}只幻翼`);
    } else {
        player.sendMessage('§7半径30格内未发现可清除的幻翼');
    }
}
