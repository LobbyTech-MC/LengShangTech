...
function onUse(evt) {
    ...
    /* ===== 清除“普通”苦力怕 ===== */
    const Creeper = Packages.org.bukkit.entity.Creeper;
    let clearCount = 0;

    world.getNearbyEntities(location, RANGE, RANGE, RANGE).forEach(function(ent) {
        if (!(ent instanceof Creeper)) return;          // 只处理苦力怕
    ...
    // 反馈信息
    if (clearCount > 0) {
        player.sendMessage(`§6已清除${clearCount}只苦力怕`);
    } else {
        player.sendMessage('§7半径10格内未发现可清除的苦力怕');
    }
}
