...
function onUse(evt) {
    ...
    /* ===== 清除“普通”蜘蛛 ===== */
    const Spider = Packages.org.bukkit.entity.Spider;
    let clearCount = 0;

    world.getNearbyEntities(location, RANGE, RANGE, RANGE).forEach(function(ent) {
        if (!(ent instanceof Spider)) return;          // 只处理蜘蛛
    ...
    // 反馈信息
    if (clearCount > 0) {
        player.sendMessage(`§6已清除${clearCount}只蜘蛛`);
    } else {
        player.sendMessage('§7半径10格内未发现可清除的蜘蛛');
    }
}
