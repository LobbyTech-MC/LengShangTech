...
function onUse(evt) {
    ...
    /* ===== 清除“普通”骷髅 ===== */
    const Skeleton = Packages.org.bukkit.entity.Skeleton;
    let clearCount = 0;

    world.getNearbyEntities(location, RANGE, RANGE, RANGE).forEach(function(ent) {
        if (!(ent instanceof Skeleton)) return;          // 只处理骷髅
    ...
    // 反馈信息
    if (clearCount > 0) {
        player.sendMessage(`§6已清除${clearCount}只骷髅`);
    } else {
        player.sendMessage('§7半径10格内未发现可清除的骷髅');
    }
}
