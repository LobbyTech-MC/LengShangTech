function onUse(event) {
  ...
  if (entity instanceof org.bukkit.entity.Item) {
    ...
    if (sfItem !== null) {
      ...
      // 检测是否为自身（便携式复刻坤）
      if (itemId === "LENGSHANG_BXSFKK") {
        player.sendMessage("§c无法复制自身！§e可恶的神人，你想无限套娃？？");
        return;
      }

      // 检测是否为自身（便携式复刻坤）
      if (itemId === "LENGSHANG_BXSFKK2") {
        player.sendMessage("§c无法复制自身！§e可恶的神人，你想无限套娃？？");
        return;
      }
    ...
    // 不可堆叠物品只给 1 个，其余仍给 64
    let copyCount = item.getType().getMaxStackSize() === 1 ? 1 : 64;
    
    let clone = item.clone();
    clone.setAmount(copyCount);
    world.dropItemNaturally(entity.getLocation(), clone);
    player.sendMessage("§d成功复制" + copyCount + "个物品！");
  } else {
